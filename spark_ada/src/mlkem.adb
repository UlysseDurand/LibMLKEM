...
is
   ...
   begin
      ...
      pragma Assert (B*k1 + B*k2 <= abs(C) + abs(D));
      pragma Assert (B*k1 + B*k2 >= -abs(C) - abs(D));
      pragma Assert (abs(C+D) <= abs(C) + abs(D) );
      pragma Assert (-abs(C)-abs(D) <= -abs(C+D));
      pragma Assert (B* (k1+k2) <= )
      pragma Assert (
         In_Range(Interval'(-abs(C+D), abs(C+D)), k1+k2)
      );
      pragma Assert (By(
         C+D mod B = 0,
         CorrespDividTwo(C + D,B)
      ));
      return True;
   end; 
